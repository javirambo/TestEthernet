...
ESP32RMTController::ESP32RMTController(int DATA_PIN, int T1, int T2, int T3, int maxChannel, int memBlocks)
    : mPixelData(0), mSize(0), mCur(0), mBufSize(0), mWhichHalf(0), mBuffer(0), mBufferSize(0), mCurPulse(0)
{
...
void ESP32RMTController::init(gpio_num_t pin)
{
    ...
    if (!FASTLED_RMT_BUILTIN_DRIVER)
    {
        ...
        if (gRMT_intr_handle == NULL) esp_intr_alloc(ETS_RMT_INTR_SOURCE, ESP_INTR_FLAG_IRAM | ESP_INTR_FLAG_LEVEL3, interruptHandler, 0, &gRMT_intr_handle);
    }
