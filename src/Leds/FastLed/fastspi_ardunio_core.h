...
class ArdunioCoreSPIOutput
{
   public:
    ...
    void inline release() __attribute__((always_inline)) { _SPIObject.endTransaction(); }
    ...
    void inline writeByte(uint8_t b) __attribute__((always_inline)) { _SPIObject.transfer(b); }

    // write a word out via SPI (returns immediately on writing register)
    void inline writeWord(uint16_t w) __attribute__((always_inline)) { _SPIObject.transfer16(w); }
