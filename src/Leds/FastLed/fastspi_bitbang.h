...
class AVRSoftwareSPIOutput
{
   ...
   public:
    ...
    #define CLOCK_LO_DELAY                                                           \
        do                                                                           \
        {                                                                            \
            delaycycles<((SPI_SPEED > 10) ? ((SPI_SPEED - 6) / 2) : (SPI_SPEED))>(); \
        } while (0);
#endif
   ...
   private:
    ...
    template <uint8_t BIT>
    __attribute__((always_inline)) inline static void writeBit(uint8_t b, clock_ptr_t clockpin, data_ptr_t datapin, data_t hival, data_t loval, clock_t hiclock, clock_t loclock)
    {
    ...
    /// combinations
    template <uint8_t BIT>
    __attribute__((always_inline)) inline static void writeBit(uint8_t b, data_ptr_t clockdatapin, data_t datahiclockhi, data_t dataloclockhi, data_t datahiclocklo, data_t dataloclocklo)
    {
#if 0
		writeBit<BIT>(b);
#else
   ...
   public:
    ...
    static void writeBytesValueRaw(uint8_t value, int len)
    {
        ...
        // Weird things may happen if software bitbanging SPI output and other pins on the output reigsters are being twiddled.  Need
