...
esp_eth_mac_t* enc28j60_begin(int MISO_GPIO, int MOSI_GPIO, int SCLK_GPIO, int CS_GPIO, int INT_GPIO, int SPI_CLOCK_MHZ, int SPIHOST)
{
    ...
    if (ESP_OK != spi_bus_initialize(SPIHOST, &buscfg, SPI_DMA_CH_AUTO)) return NULL;
    ...
    if (ESP_OK != spi_bus_add_device(SPIHOST, &devcfg, &spi_handle)) return NULL;
