...
extern "C"
{
    esp_eth_mac_t *enc28j60_begin(int MISO, int MOSI, int SCLK, int CS, int INT, int SPICLOCK_MHZ, int SPIHOST);
...
ESP32_ENC::ESP32_ENC() : initialized(false), staticIP(false), eth_handle(NULL), started(false), eth_link(ETH_LINK_DOWN) {}
...
bool ESP32_ENC::begin(int MISO, int MOSI, int SCLK, int CS, int INT, int SPICLOCK_MHZ, int SPIHOST, uint8_t *ENC28J60_Mac)
{
    tcpipInit();
    ...
    if (esp_read_mac(mac_eth, ESP_MAC_ETH) == ESP_OK)
    {
        ...
        sprintf(macStr, "%02X:%02X:%02X:%02X:%02X:%02X", mac_eth[0], mac_eth[1], mac_eth[2], mac_eth[3], mac_eth[4], mac_eth[5]);
...
bool ESP32_ENC::setHostname(const char *hostname) { return tcpip_adapter_set_hostname(TCPIP_ADAPTER_IF_ETH, hostname) == 0; }
...
bool ESP32_ENC::enableIpV6() { return tcpip_adapter_create_ip6_linklocal(TCPIP_ADAPTER_IF_ETH) == 0; }
